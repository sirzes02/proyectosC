	...
	public:
		...
		void FillTriangle(int x1, int y1, int x2, int y2, int x3, int y3, short c = 0x2588, short col = 0x000F){
			...
			next2:
				...
				drawline(minx, maxx, y);    // Draw line from min to max points found on the yNow increase y
				if (!changed1){
					t1x += signx1;
				}	
				t1x += t1xp;
		...
		class olcAudioSample{
			public:
				...
				olcAudioSample(std::wstring sWavFile){
					...
					std::fread(&dump, sizeof(char), 4, f); // Not Interested
					std::fread(&wavHeader, sizeof(WAVEFORMATEX) - 2, 1, f); // Read Wave Format Structure chunk Note the -2, because the structure has 2 bytes to indicate its own size which are not in the wav file
